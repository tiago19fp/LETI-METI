...
class Client:
    ...
    def process(self, msg=b""):
        ...
        key = ks_client.read("key")

        rsa_public_key = serialization.load_pem_public_key(key[:450],backend=default_backend())
        rsa_private_key = serialization.load_pem_private_key(key[450:],password = b'grupo02client',backend=default_backend())
        ...
        if(self.msg_cnt == 2): 
            ...
            msg = msg[256:]
            #print(signature)
            rsa_public_key.verify(signature,msg,padding.PSS(mgf=padding.MGF1(hashes.SHA256()),salt_length=padding.PSS.MAX_LENGTH),hashes.SHA256())
            ...
            signature = rsa_private_key.sign(encryptMessage,padding.PSS(mgf=padding.MGF1(hashes.SHA256()),salt_length=padding.PSS.MAX_LENGTH),hashes.SHA256())
            return signature + encryptMessage if len(new_msg)>0 else None
