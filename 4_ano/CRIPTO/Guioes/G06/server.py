...
class ServerWorker(object):
    ...
    def process(self, msg):
        ...
        key = ks_server.read("key")

        rsa_public_key = serialization.load_pem_public_key(key[:450],backend=default_backend())
        rsa_private_key = serialization.load_pem_private_key(key[450:],password = b'grupo02server',backend=default_backend())
        ...
        if(self.msg_cnt == 1):
            ...
            signature = rsa_private_key.sign(new_msg,padding.PSS(mgf=padding.MGF1(hashes.SHA256()),salt_length=padding.PSS.MAX_LENGTH),hashes.SHA256())
            #print(signature)
            #print(len(signature))
              
            self.key1=derived_key[0:16]
            return signature + new_msg if len(new_msg) > 0 else None
