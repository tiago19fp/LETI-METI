...
class Client:
    ...
    def process(self, msg=b""):
        ...
        if(self.msg_cnt == 2):
            ...
            signature = msg[:256]
            certificado_recebido = msg[256:1363]
            msg = msg[1363:]
            # print(signature)
            self.ca_cert = load_ca_cert("EC_self_signed.cer")
            ...
            c_sk_c = serialization.load_pem_private_key(
                c_sk_pem, password=None, backend=default_backend())
            print(len(certificado))
            server_certificate = crypto.load_certificate(
                crypto.FILETYPE_PEM, certificado_recebido)
            public_rsa = server_certificate.get_pubkey().to_cryptography_key()

            public_rsa.verify(signature, msg, padding.PSS(mgf=padding.MGF1(
                hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA256())

            server_certificado = crypto.load_certificate(
                crypto.FILETYPE_PEM, certificado_recebido)
            try:
            ...
            signature = c_sk_c.sign(encryptMessage, padding.PSS(mgf=padding.MGF1(
                hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA256())
