...
int main(int argc, char *argv[])
{
	...
	int sistemaLDR = open("sistemaCentralLDR.csv", O_CREAT | O_RDWR | O_APPEND, 0600);
	char bufWritLogSta[1024];
	uint8_t buffer[3000]; //data buffer of 1K
				...
				else
				{
					...
					if (buffer[0] == 2)
					{
						...
						for (int c = 0; c < nSamples; c++)
						{
							...
							sprintf(bufWritLogSta, "%d,%.1f,%s", (int)buffer[apontador + counter], lux, ctime(&timestamp));
