				...
				else
				{
					...
					//{
						...
						if (buffer[0] == 2)
						{
							...
							int nISS = 0;
							char query[2048];
							sprintf(query,"INSERT INTO Concentrador Values(%d,2,3);",(int)buffer[1]);
							...
							for(int count = 0;count < nISS;count++){
								...
								sprintf(query,"INSERT INTO Iss Values(%d,%d,%d,%d);",issNumber,issX,issY,(int)buffer[1]);
								int res = mysql_query(conn, query);
								write(sistemaGPS, bufWritLogSta3, strlen(bufWritLogSta3));
								cont += 3;
							}
							...
							for (int c = 0; c < nSamples; c++)
							{
								...
								sprintf(query,"INSERT INTO AmostraLux Values(0,%d,%d,%.1f,FROM_UNIXTIME(%ld));",(int)buffer[apontador + counter], (int)buffer[1], lux, timestamp);
								int res = mysql_query(conn, query);
								counter += 9;
							}
						}
						if (buffer[0] == 3)
						{
							...
							for (int c = 0; c < nSamples; c++)
							{
								...
								sprintf(query,"INSERT INTO AmostraMov Values(0,%d,%d,FROM_UNIXTIME(%ld));",(int)buffer[7 + counter],(int)buffer[1],timestamp);
								int res = mysql_query(conn, query);
